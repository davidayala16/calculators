...
// Switches to Advanced mode, opens every collapsible section, and selects "Custom" carb rate so
// every numeric input in the app is mounted in the DOM. Update this list if App.jsx's section
// headers change.
export async function expandEverything(page) {
  ...
  const collapsedByDefaultHeaders = [
    'SWEAT & SODIUM SETTINGS', 'RECIPE BASE', 'DOSING & PACKAGING',
  ];
  for (const h of collapsedByDefaultHeaders) {
    const btn = page.locator(`button.cg-collapsible-header:has-text("${h}")`).first();
    if (await btn.count()) {
      await btn.click();
      await page.waitForTimeout(80);
    }
